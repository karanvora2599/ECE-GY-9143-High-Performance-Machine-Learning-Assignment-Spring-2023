...
int main() {
    ...
    // ... [Kernel executions and timings as before]
    // execute simple convolution kernel
    auto start = std::chrono::high_resolution_clock::now();
    simpleConvolution<<<dim3(K, (W + 15) / 16, (H + 15) / 16), dim3(16, 16, 1)>>>(d_I, d_F, d_O);
    cudaDeviceSynchronize();
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> duration = end - start;
    std::cout << "C1_" << checksum(d_O, K * W * H) << "," << std::fixed << std::setprecision(3) << duration.count() << std::endl;
    ...
    duration = end - start;
    std::cout << "C2_" << checksum(d_O, K * W * H) << "," << std::fixed << std::setprecision(3) << duration.count() << std::endl;
    ...
    duration = end - start;
    std::cout << "C3_" << checksum(d_O, K * W * H) << "," << std::fixed << std::setprecision(3) << duration.count() << std::endl;
